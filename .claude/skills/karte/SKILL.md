...
- **行の特定**: `⋯` ボタンから親をたどり、**7桁の病名コードをちょうど1つ含む要素**を行とみなす。テキストに病名を含むかで探すと、1件しかない区画（継続中／中止）では別の行を拾う（2026-09-26、主病を「発熱」に設定したつもりが中止済みの「インフルエンザの疑い」に付いた）
- **ダイアログの病名一覧に★は出ない**。主病かどうかは `⋯` メニューが「主病に設定」か「主病解除」か、または API の `isMain` で判断する
- **日付の選択**: 日付ボタンを押したあと、カレンダーが開くまで少し待つ。1回目で開かず日付が空（「日付形式が不正です」）になることがあるので、選んだ後に `2026年MM月DD日` が入ったか確認し、入っていなければ日付ボタンから押し直す。日付は `button[aria-label*="2026年9月26日"]` で押すと確実（数字だけで探すと前後の月のセルを拾う）
...
- **拡張の接続が切れたとき**: 数十秒で元のタブグループごと戻ることが多い。`tabs_context_mcp` を `createIfEmpty` なしで呼んで確認する。`createIfEmpty: true` で作ったタブは別のブラウザ側に開いてサインイン画面になることがある（元のタブはログインしたまま）
  - 戻らないときは `list_connected_browsers` で使用中のブラウザを確認する。ブラウザ名（Browser 1/2）は接続し直すたびに入れ替わるので、deviceId で判断する
  - 接続が切れると保存前の編集は消える。再接続後に版と内容を読み直し、足りない分を入れ直す
  - タブを何枚も並行で開くより、1枚のタブで患者を順に回るほうが切断の影響が小さい
- **ヘルパーの使い回し**: 関数群を `fn.toString()` で文字列にして `localStorage` に保存し、ページを開くたびに `(0,eval)(localStorage.getItem(...))` で読み込むと、患者ごとに長いスクリプトを送り直さずに済む
