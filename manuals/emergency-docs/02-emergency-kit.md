...
> 薬剤の選択・投与量の最終判断は医師が行う。本マニュアルは所在と標準的な用量の目安を示すもので、実際の投与は必ず医師の指示に従い、添付文書を確認すること。

**薬剤名をクリックすると PMDA（医薬品医療機器総合機構）の添付文書ページが開く。** 同じ成分の全製品が一覧表示されるので、当院採用の販売名の行の「PDF」または「HTML」を開くこと。改訂されてもリンクは切れない。
